<script lang="ts">
  import { chatStore, connectionStore, agentsStore, skillStore, explorerStore } from "$lib/stores";
  ...
  import { getFileSystem, getFileName, getExtension, isImageFile } from "$lib/filesystem";
  ...
  let selectedAgentLabel = $derived.by(() => {
    ...
    if (agentsStore.agents.length > 0) return "Default agent";
    return "";
  });
  ...
  export async function addExplorerFiles(paths: string[]) {
    for (const filePath of paths) {
      if (attachedFiles.length >= 10) break;
      try {
        const fs = getFileSystem(explorerStore.currentSource);
        ...
        if (isImageFile(ext)) {
          ...
          try {
            ...
            if (isTextPreviewable(ext)) {
              const text = await fs.readFileText(filePath);
              if (text) {
