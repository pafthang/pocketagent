import { getFileSystem, type FileSource } from "$lib/filesystem";
...
export async function getTextPreview(
  ...
  extension: string,
  size: number,
  opts?: { source?: FileSource; id?: string },
): Promise<string | null> {
  ...
  const promise = (async (): Promise<string | null> => {
    await acquire();
    try {
      const fs = getFileSystem(opts?.source ?? "remote");
      const text = await fs.readFileHead(path, MAX_HEAD_BYTES, opts?.id ? { id: opts.id, path, isDir: false, name: "", source: opts.source ?? "remote" } : undefined);
