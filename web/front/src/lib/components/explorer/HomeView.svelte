<script lang="ts">
  ...
  import type { RecentFileEntry } from "@pocketagent/client";
  import {
  ...
  let spaceFilesAvailable = $derived(
    connectionStore.isAuthenticated && connectionStore.isConnected,
  );
  ...
  onMount(async () => {
    ...
    try {
      ...
      const res = await client.files.recent(15);
      recentFiles = res.files;
    } catch {
      // ignore
    }
  });
  ...
  function openRecent(entry: RecentFileEntry) {
    if (entry.is_dir) {
      explorerStore.navigateTo(entry.path, "remote");
    } else {
      explorerStore.openFileByPath(entry.path);
    }
  }

  function openSpaceFiles() {
    explorerStore.navigateTo("/", "remote");
  }

  async function openLocalFolder() {
    const ok = await pickBrowserLocalDirectory();
    if (ok) {
      await explorerStore.switchSource("local");
      toast.success(`Opened local folder: ${getBrowserLocalRootName()}`);
    }
  }
</script>
...
<div class="space-y-6 overflow-y-auto p-5">
  ...
  <!-- Space files (PocketAgent Files API) -->
  <section>
    <h2 class="mb-2.5 text-[11px] font-medium uppercase tracking-wider text-muted-foreground/70">
      Space files
    </h2>
    <div class="grid grid-cols-[repeat(auto-fill,minmax(180px,1fr))] gap-2">
      <button
        type="button"
        disabled={!spaceFilesAvailable}
        class="flex items-center gap-2.5 rounded-lg border border-border/50 bg-card/30 px-3 py-2.5 text-left transition-colors hover:bg-muted/50 disabled:cursor-not-allowed disabled:opacity-40"
        onclick={openSpaceFiles}
      >
        <Server class="h-5 w-5 shrink-0 text-emerald-400" />
        <div class="min-w-0">
          <p class="truncate text-sm text-foreground">PocketAgent storage</p>
          <p class="truncate text-[11px] text-muted-foreground/60">
            {spaceFilesAvailable ? "Browse space files" : "Connect to backend"}
          </p>
        </div>
      </button>
    </div>
  ...
  <section>
    ...
    <div class="grid grid-cols-[repeat(auto-fill,minmax(180px,1fr))] gap-2">
      {#if browserLocalPickerSupported()}
        <button
          type="button"
          class="flex items-center gap-2.5 rounded-lg border border-border/50 bg-card/30 px-3 py-2.5 text-left transition-colors hover:bg-muted/50"
        ...
        >
          ...
          <div class="min-w-0">
            <p class="truncate text-sm text-foreground">
              {hasBrowserLocalDirectory() ? getBrowserLocalRootName() : "Open local folder"}
            </p>
            <p class="truncate text-[11px] text-muted-foreground/60">Browser File System Access</p>
          </div>
        </button>
      {/if}
      {#each localFolders as folder}
        <button
          type="button"
          class="flex items-center gap-2.5 rounded-lg border border-border/50 bg-card/30 px-3 py-2.5 text-left transition-colors hover:bg-muted/50"
          onclick={() => explorerStore.navigateTo(folder.path, "local")}
        >
          <Folder class="h-5 w-5 shrink-0 text-yellow-400" />
          <div class="min-w-0">
            <p class="truncate text-sm text-foreground">{folder.name}</p>
