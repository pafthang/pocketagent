<script lang="ts">
  ...
  import { getFileSystem, entryContext } from "$lib/filesystem";
  ...
  let items = $derived.by((): MenuEntry[] => {
    if (!file) {
      ...
      return [
          ...
          action: () => {
            ...
            input.onchange = () => {
              if (input.files?.length) {
                void explorerStore.uploadFiles(Array.from(input.files));
              }
            };
            input.click();
            onClose();
        ...
        "separator",
        {
          ...
          icon: Terminal,
          action: async () => {
            await fs.openInTerminal(explorerStore.currentPath);
            onClose();
          },
        },
      ];
    ...
    if (file.isDir) {
      ...
      result.push({
        ...
        icon: Terminal,
        action: async () => {
          await fs.openInTerminal(file.path);
          onClose();
        },
      });
    }
    ...
    result.push({
      ...
      action: async () => {
        if (confirm(`Delete "${file.name}"?`)) {
          try {
            await fs.deleteFile(file.path, file.isDir, entryContext(file));
            explorerStore.refresh();
          } catch {
            // Error handling
          }
