import type { FileEntry, DefaultDirs, FileChangeEvent, FileSource, FileSystemProvider } from "$lib/filesystem";
import {
...
function createTab(path = "", source: FileSource = defaultExplorerSource()): ExplorerTab {
  return {
...
class ExplorerStore {
  ...
  isHome = $derived.by(() => {
    if (this.currentSource === "remote") return isVirtualRoot(this.currentPath);
  ...
  // ─── Private Helpers ───────────────────────────────────────
  private fs(source?: FileSource): FileSystemProvider {
    return getFileSystem(source ?? this.currentSource);
  }
  ...
  // ─── Tab Management ────────────────────────────────────────
  createNewTab(path = "", source: FileSource = defaultExplorerSource()): string {
  ...
  private async onTabActivated(): Promise<void> {
    ...
    if (tab.path) {
      ...
      try {
        tab.isLoading = true;
        tab.files = await this.fs(tab.source).readDir(tab.path);
        tab.isLoading = false;
      } catch {
        tab.isLoading = false;
      }
  ...
  private async loadTabDirectory(tabId: string, path: string): Promise<void> {
    ...
    tab.error = null;
    try {
      tab.files = await this.fs(tab.source).readDir(path);
  ...
  performRecursiveSearch(): void {
    ...
    this.recursiveSearchTimer = setTimeout(async () => {
      ...
      try {
        const result = await this.fs(currentTab.source).searchRecursive(
          currentTab.path,
  ...
  async initialize(): Promise<void> {
    ...
    const source = defaultExplorerSource();
    try {
      this.defaultDirs = await this.fs(source).getDefaultDirs();
    } catch {
      // leave null
    }

    if (this.tabs.length === 0) {
      const homePath = source === "remote" || hasBrowserLocalDirectory() ? "/" : "";
      const tab = createTab(homePath, source);
      this.tabs = [tab];
      this.activeTabId = tab.id;
      if (homePath) {
        await this.loadTabDirectory(tab.id, homePath);
      }
    }
  }
  ...
  async navigateTo(path: string, source?: "local" | "remote" | "cloud"): Promise<void> {
    ...
    try {
      ...
      const entries = await this.fs(s).readDir(path);
      this.updateActiveTab((tab) => {
        tab.files = entries;
        tab.path = path;
        tab.source = s;
  ...
  goHome(): void {
    const homePath =
      this.currentSource === "remote" || hasBrowserLocalDirectory() ? "/" : "";
    this.updateActiveTab((tab) => {
      tab.path = homePath;
      tab.files = [];
      tab.openFile = null;
      ...
      tab.recursiveResults = [];
      tab.typeFilters = new Set();
      tab.history = homePath ? [homePath] : [];
      tab.historyIndex = homePath ? 0 : -1;
    });
    this.stopWatching();
    if (homePath) void this.refresh();
  }
  ...
  async paste(): Promise<void> {
    ...
    const fs = this.fs();
    if (!fs.supportsPaste) {
      toast.info("Paste is not supported for this file source");
      return;
    }
    try {
      for (const srcPath of this.clipboardFiles) {
        ...
        if (this.clipboardMode === "copy") {
          if (stat.isDir) {
            await fs.copyDir(srcPath, destPath);
          } else {
  ...
  async commitRename(oldPath: string, newName: string): Promise<void> {
    ...
    if (!fs.supportsRename) {
      toast.info("Rename is not supported for remote files");
      this.renamingFile = null;
      return;
    }
    try {
      ...
      const entry = this.files.find((f) => f.path === oldPath);
      await fs.rename(oldPath, newPath, entry);
      this.renamingFile = null;
      await this.refresh();
    } catch (e) {
  ...
  async createFolder(name: string): Promise<void> {
    ...
    try {
      ...
      await this.fs().createDir(newPath);
      await this.refresh();
    } catch (e) {
  ...
  private async loadWithoutHistory(path: string): Promise<void> {
    ...
    try {
      const entries = await this.fs(this.currentSource).readDir(path);
      this.updateActiveTab((tab) => {
        tab.files = entries;
        tab.path = path;
        tab.isLoading = false;
  ...
  async deleteSelected(): Promise<void> {
    const tab = this.activeTab;
    if (!tab || tab.selectedFiles.size === 0) return;
    const fs = this.fs(tab.source);

    for (const path of tab.selectedFiles) {
      const entry = tab.files.find((f) => f.path === path);
      try {
        await fs.deleteFile(path, entry?.isDir ?? false, entry);
      } catch (e) {
  ...
  async uploadFiles(files: File[]): Promise<void> {
    const tab = this.activeTab;
    if (!tab?.path || files.length === 0) return;
    const fs = this.fs(tab.source);
    if (!fs.uploadFiles) {
      toast.info("Upload is not available for this file source");
      return;
    }
    try {
      await fs.uploadFiles(tab.path, files, tab.files[0]?.projectId);
      await this.refresh();
  ...
  async switchSource(source: FileSource): Promise<void> {
    const homePath = source === "remote" || hasBrowserLocalDirectory() ? "/" : "";
    this.updateActiveTab((tab) => {
      tab.source = source;
      tab.path = homePath;
    ...
    if (homePath) await this.navigateTo(homePath, source);
    else this.goHome();
  }
  ...
  private async startWatching(path: string): Promise<void> {
    this.stopWatching();
    try {
      this.unwatchFn = await this.fs().watch(path, (event: FileChangeEvent) => {
